...
"""
def spherical_camera(radius: float, elev_deg: float, azim_deg: float) -> np.ndarray:
...
"""
def prepare_mesh_for_render(mesh: o3d.geometry.TriangleMesh) -> tuple[o3d.geometry.TriangleMesh, float]:
...
def clip_mesh_at_layer(
    ...
    z_cut = z_min + layer_number * layer_height
    print(f"[DEBUG] layer={layer_number} | z_min={z_min:.3f} | z_cut={z_cut:.3f} | z_max={z_max:.3f}")
    ...
    clipped = trimesh_to_open3d(clipped_tm)
    return clipped, z_cut


def make_renderer(width: int, height: int, bg=(1.0, 1.0, 1.0, 1.0)):
...
def render_view_with_renderer(
    ...
    render_mesh, radius = prepare_mesh_for_render(mesh)
    ...
    eye = spherical_camera(radius=radius, elev_deg=elev, azim_deg=azim)
    up = np.array([0.0, 0.0, 1.0], dtype=float)
...
def main():
    ...
    if args.single:
        ...
        views = {
            "cad_view": (28, -45),
            "front": (0, -90),
            "right_front_top": (22, -35),
            "left_front_top": (22, -135),
            "top_front": (65, -45),
            "top_right": (65, -15),
        }
    """
